...
class LP:
        ...
        for name, s in self.states.items():
            for action in s.actions:
                ...
                for next_state in next_states:
                    a[getidx_of_state(next_state.dest)][idx] -= next_state.prob
    ...
    def get_alpha(self):
        ...
        s = State(HEALTH_VALUES[-1], ARROWS_VALUES[-1], STAMINA_VALUES[-1]).get_hash() ## change??
        alpha[s][0] = 1
        return alpha
    ...
    def get_policy(self):
        ...
        for name, s in self.states.items():
            ...
            local.append(ACTION_NAMES[best_action]) ## change??
